...
public static class MassGomokuE2EScenario
{
    ...
    public static async Task<bool> RunAsync(bool interactive = false)
    {
        ...
        for (int i = 1; i <= USER_COUNT; i++)
        {
            ...
            try { await Task.Delay(SPAWN_INTERVAL_MS, ct); }
    ...
    private static UserType Classify(int uid)
    {
        ...
        if (h < FO_A)           return UserType.FailoverA;
        if (h < FO_A + FO_B)    return UserType.FailoverB;
        if (h < FO_A + FO_B + FO_C) return UserType.FailoverC;
    ...
    private static async Task RunUserAsync(int uid, UserType type, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (!await EnterQueueAsync(http, ct)) { Interlocked.Increment(ref _queueFail); return; }
            ...
            if (!activated) { Interlocked.Increment(ref _activeFail); return; }
            ...
            try { await RunGameStageAsync(uid, type, session, http, ct); }
        ...
        int uid, UserType type, TokenSession session, HttpClient http, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var recvTask = Task.Run(async () =>
            {
                try
                {
                    while (true)
                    {
                        ...
                        {
                            case Packet.PayloadOneofCase.SLogin:       break;
                            case Packet.PayloadOneofCase.SGameStart:   startTcs.TrySetResult(env.SGameStart); break;
                            case Packet.PayloadOneofCase.SBoardUpdate: break;
                            case Packet.PayloadOneofCase.SGameOver:    overTcs.TrySetResult(env.SGameOver); return;
                        }
                    }
                }
                catch { }
            ...
            try { gameStart = await startTcs.Task.WaitAsync(startLinked.Token); }
            ...
            if (overTcs.Task.IsCompletedSuccessfully)
            {
                ...
                if (over.WinnerId == session.PlayerId)     Interlocked.Increment(ref _win);
                else if (over.WinnerId == 0)               Interlocked.Increment(ref _draw);
                else                                       Interlocked.Increment(ref _lose);

                // Stage 10 — MatchRecord 검증
                AuthHelper.ApplyToken(http, session);
                bool verified = await VerifyMatchRecordAsync(http, ct);
                if (verified) Interlocked.Increment(ref _verifyOk);
                else          Interlocked.Increment(ref _verifyFail);
            }
            else
            {
        ...
        finally
        {
            if (lobby != null)
            {
                try { await lobby.StopAsync(CancellationToken.None); } catch { }
                try { await lobby.DisposeAsync(); } catch { }
            }
        }
    }
