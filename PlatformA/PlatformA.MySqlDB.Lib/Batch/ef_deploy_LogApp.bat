@echo off
:: ============================================================
:: [배포용] db_LogApp 마이그레이션 적용 — 기존 데이터 보존
::
:: 역할:
...
::   3) 이 스크립트를 실행합니다.
::
:: 적용 대상 테이블: access_logs
:: ============================================================

:: ── 연결 정보 (환경에 맞게 수정하세요) ──────────────────────
set DB_SERVER=localhost
set DB_PORT=3306
set DB_NAME=db_LogApp
set DB_USER=root
...
if not exist "Migrations\LogApp" (
    echo.
    echo [ERROR] Migrations\LogApp 폴더가 없습니다.
    echo         아직 마이그레이션이 생성된 적 없는 환경입니다.
    echo.
    echo         해결 방법: ef_create_database_CF_LogApp.bat 을 먼저 실행하세요.
...
echo.
echo [LogApp Deploy] 연결 대상: %DB_SERVER%:%DB_PORT%/%DB_NAME%
echo [LogApp Deploy] 미적용 마이그레이션 적용 중...
...
echo.
echo [LogApp Deploy] 완료. 모든 마이그레이션이 적용되었습니다.
pause
