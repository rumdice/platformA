...
#### ⚠️ 기술 도입 체크리스트 (C# 코드 변경 여부와 무관하게 먼저 확인)

아래 중 하나라도 해당하면 판정은 무조건 `📝 신규 ADR 필요`다:

- 기존에 없던 외부 서비스·툴이 도입되는가?
  (DB, 메시지 브로커, 워크플로 엔진, 캐시, 검색엔진, 모니터링 등)
- 새로운 데이터 저장소 인스턴스가 추가되는가?
  (신규 DB 컨테이너, 스토리지 버킷, 파일 시스템 마운트 등)
- 인프라 아키텍처에 새 컴포넌트가 추가되는가?
  (서비스 mesh, API 게이트웨이, 로드밸런서, CDN 등)
- 기존 통신 패턴과 다른 새 패턴이 사용되는가?
  (REST→gRPC, HTTP→WebSocket, 동기→비동기 등)

> **판단 기준 요약**: ADR 필요 여부는 C# 코드 변경이 아닌
> "기존 아키텍처에 없던 설계 결정이 포함되는가"로 판단한다.
