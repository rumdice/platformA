...
n8n: GitHub CI Failure Monitor (10분 폴링)
    ...
    └─ 실패 유형 분류 (JS Code 노드)
    └─ Job Lock claim (로컬 PostgreSQL에서 직접 수행)
    └─ GitHub: repository_dispatch (ai-auto-fix 이벤트)
    └─ PostgreSQL: sdlc.ai_failures INSERT
         (ON CONFLICT DO NOTHING — 중복 방지)
```

> **Job Lock**: n8n은 로컬 DB에 직접 접근할 수 있으므로 Job Lock claim을 GitHub Actions가 아닌 n8n이 수행한다.
> GitHub Actions에서는 로컬 PostgreSQL에 접근할 수 없다 (보안 경계).
...
| 7 | Job Lock claim | 로컬 PostgreSQL에서 직접 수행 |
| 8 | GitHub: repository_dispatch | auto-fix 이벤트 트리거 |
| 9 | PostgreSQL: ai_failures INSERT | ON CONFLICT 중복 방지 INSERT |
