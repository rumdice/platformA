...
## 태스크
- [ ] Matching.API: MatchingHub.cs 삭제 + Program.cs MapHub 제거
- [ ] Matching.API: RequestMatch(Deprecated) 엔드포인트 제거
- [ ] Matching.API: CancelMatch(JWT) → POST /cancel (내부 전용) 교체
- [ ] Matching.API: GetStatus(JWT) → GET /status/{userId} (내부 전용) 교체
- [ ] LobbyHub: JWT 포워딩 방식 → 내부 엔드포인트 호출로 교체
- [ ] DummyClient: LoadTestMatchingScenario 삭제 (Scenario 10 대체)
- [ ] DummyClient: MatchingScenario → Lobby SignalR 경유로 재작성
- [ ] GameMatchControllerTests: 삭제된 엔드포인트 테스트 제거
- [ ] Consts.cs: MATCH_API_URL, MATCH_HUB_URL 상수 제거
