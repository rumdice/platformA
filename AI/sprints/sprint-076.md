...
## 태스크
- [ ] 게임 결과 저장 수정: GomokuRoom→Matching.API 파이프라인 디버그 및 재시도 로직 추가
- [ ] MMR/ELO 시스템 구현: PlayerRating 엔티티 + ELO 계산 + 게임 종료 시 자동 업데이트
- [ ] Rate Limit 재설계: 로그인 IP→유저 기반으로 전환, 임계값 현실화(50~100/분)
- [ ] Game.Gomoku 헬스체크 추가: /healthz(TCP liveness) + /readyz(Matching.API 연결 확인)
- [ ] DummyClient ServiceManager 개선 사항(FindRepoRoot + --no-build) 커밋
