...
Plan mode 진입 시 **신규 계획 파일을 생성**한다. 시스템이 제공하는 랜덤 이름 파일(예: `bright-discovering-bear.md`)을 재사용하거나 덮어쓰지 않는다.

- **신규 파일 생성**: 태스크마다 `.claude/plans/YYYY-MM-DD_PlanName.md` 형식으로 새 파일을 `Write` 툴로 생성한다.
  - `YYYY-MM-DD`: 오늘 날짜
  - `PlanName`: 사용자 요청에서 PascalCase로 자동 생성
  - 예시: `.claude/plans/2026-05-12_MatchingImprovement.md`
- **시스템 제공 파일 무시**: 시스템이 제공하는 랜덤 이름 파일은 사용하지 않는다. 신규 명명 파일에 직접 작성한다.
- **이전 플랜 파일 보존**: 이전 태스크의 플랜 파일을 삭제하거나 수정하지 않는다.
